...
public:
    ...
    Cliente() : nit(""), nombre(""), direccion("") {} // Constructor por defecto
...
public:
    ...
    Producto() : codigo(""), nombre(""), marca(""), precio(0.0) {} // Constructor por defecto
...
public:
    ...
    void imprimirFactura() {
        ...
        cout << "------------------------------------------------------------\n";
        cout << "| Cod  | Producto                     | Marca     | Precio  | Cantidad |\n";
        cout << "------------------------------------------------------------\n";
        for (auto& d : detalles) {
            cout << " " << setw(4) << d.producto.codigo
                << " | " << setw(28) << left << d.producto.nombre
                << "| " << setw(10) << left << d.producto.marca
                << "| Q " << fixed << setprecision(2) << setw(6) << d.producto.precio
                << " | " << d.cantidad << "\n";
        }
        cout << "------------------------------------------------------------\n";
...
Producto buscarProducto(string codigo) {
    for (auto& p : productos)
        if (p.codigo == codigo)
            return p;
    return Producto(); // producto vacío
}
...
void crearVenta() {
    ...
    while (true) {
        ...
        venta.agregarDetalle(DetalleVenta(p, cantidad)); // se agrega solo una vez con la cantidad
    }
